...
// //           showPaymentPopup: loginModel.data!.showpaymentpopup ?? true,
// //         );
// //       }
...
//           showPaymentPopup: loginModel.data!.showpaymentpopup ?? true,
//         );
//         await SharedPreferenceHelper.saveSubscriptionStatus(
//           loginModel.data!.subscriptionstatus ?? '',
//         );
//       }
...
class LoginProvider extends ChangeNotifier with ProviderHelperClass {
  ...
  bool isRegisterFormValidated = false;
  bool isRememberCredentials = true;
  Future<void> login({Function(String role, bool trialExpired, String subscriptionStatus)? onSuccess, Function(String errorMessage)? onFailure}) async {
    updateLoadState(LoaderState.loading);
    var res = await serviceConfig.login(
        phone: loginUsernameController.text.trim(),
        password: loginPasswordController.text);
    if (!res.isError) {
      ...
      if (onSuccess != null && loginModel.data != null) {
        onSuccess(loginModel.data!.role.toLowerCase(), loginModel.data!.trialExpired ?? true, loginModel.data!.subscriptionstatus ?? '');
      } else if (onSuccess != null && !loginModel.status) {
        if (onFailure != null) onFailure(loginModel.message);
      }
      updateLoadState(LoaderState.loaded);
