...
// // Paid user → skip payment screen entirely
...
// // Explicitly expired → show payment screen
// if (subscriptionStatus == 'expired') {
//   _navigate(const PaymentScreen());
//   return;
// }

// // Fallback for users without a subscription_status value: use trial_expired flag
// final trialExpired = await SharedPreferenceHelper.getTrialExpired();
...
import 'package:construction_app/provider/version_provider.dart';
import 'package:construction_app/services/service_config.dart';
import 'package:construction_app/view/company/payment_subscription_screen.dart';
import 'package:construction_app/view/phone_verification_screen.dart';
import 'package:construction_app/widgets/app_theme.dart';
import 'package:flutter/material.dart';
...
class _SplashScreenState extends State<SplashScreen>
  ...
  final ServiceConfig _serviceConfig = ServiceConfig();
  ...
  /// Validates the stored token by calling the profile API.
  /// Returns false if the user is deleted, token is invalid, or any error occurs.
  Future<bool> _validateToken() async {
    try {
      final res = await _serviceConfig.profile();
      return !res.isError;
    } catch (e) {
  ...
  Future<void> _checkLoginState() async {
    ...
    // ── Validate token with backend (catches deleted users after reinstall) ──
    final bool isTokenValid = await _validateToken();
    if (!isTokenValid) {
      await SharedPreferenceHelper.clearWholeData();
      _navigate(const PhoneVerificationScreen());
      return;
    }
    ...
    final subscriptionStatus =
        await SharedPreferenceHelper.getSubscriptionStatus();
    ...
    // ── Fallback: check trial expired flag ───────────────────────────
    final trialExpired = await SharedPreferenceHelper.getTrialExpired();
    if (trialExpired) {
      _navigate(const PaymentScreen());
      return;
