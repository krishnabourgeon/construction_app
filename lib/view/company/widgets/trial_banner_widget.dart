...
//     final showPopup = await SharedPreferenceHelper.getShowPaymentPopup();
//     if (mounted) {
//       setState(() {
//         _daysLeft = daysLeft;
//         _trialExpired = expired;
//         _showPaymentPopup = showPopup;
//         _loaded = true;
//       });
//     }
//   }
...
//   @override
//   Widget build(BuildContext context) {
//     if (!_loaded || !_showPaymentPopup) return const SizedBox.shrink();
...
// ══════════════════════════════════════════════════════════════════════════════
// TRIAL BANNER WIDGET
// Driven purely by subscription_status stored in SharedPreferences:
//   'paid'    → hide banner
//   'trial'   → show trial-active banner (days left from trial_days_left)
//   'expired' → show expired banner
//   other/''  → hide banner
// ══════════════════════════════════════════════════════════════════════════════
...
class _TrialBannerWidgetState extends State<TrialBannerWidget> {
  ...
  Future<void> _loadInfo() async {
    final status = await SharedPreferenceHelper.getSubscriptionStatus();
    final daysLeft = await SharedPreferenceHelper.getTrialDaysLeft();
    if (mounted) {
      setState(() {
  ...
  void _navigateToPayment() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const PaymentScreen()),
    ).then((_) => _loadInfo()); // refresh banner on return
  }
  ...
  Widget build(BuildContext context) {
    ...
    switch (_subscriptionStatus) {
      ...
      case 'expired':
        return _ExpiredBanner(onUpgrade: _navigateToPayment);
      default:
        // 'paid' or any unknown value → show nothing
