...
class _EditMaterialScreenState extends State<EditMaterialScreen> {
  ...
  void initState() {
    ...
    // Pre-fill fields with existing material data
    _nameCtrl = TextEditingController(text: widget.material.materialName);
    _qtyCtrl = TextEditingController(text: widget.material.qty.toString());
    _priceCtrl = TextEditingController(text: widget.material.price.toString());
    _dateCtrl = TextEditingController(text: _formatDate(widget.material.addedDate));

    _selectedCategoryId = widget.material.categoryId;
    _selectedUnitId = widget.material.unitId;
    _selectedSupplierId = widget.material.supplierId;
  ...
  void _onNameChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      context.read<CompanyProvider>().getMaterialNames(null, query).then((_) {
        if (mounted) _updateSuggestions(query);
      });
    });
  }
  ...
  void _updateSuggestions(String query) {
    final apiNames =
        context.read<CompanyProvider>().materialNamesList.map((e) => e.name).toList();
    ...
    setState(() {
      ...
      _showSuggestions =
          (filtered.isNotEmpty || query.length >= 1) && _nameFocusNode.hasFocus;
    });
  }
              ...
              if (newName.isNotEmpty) {
                setState(() {
                  final apiNames = context
                      ...
                      .map((e) => e.name)
                      .toList();
                  if (!apiNames.contains(newName) &&
                      !_locallyAddedNames.contains(newName)) {
  ...
  Future<void> _pickDate() async {
    final picked = await showDatePicker(
      context: context,
      initialDate: widget.material.addedDate,
  ...
  void _save() async {
    ...
    await provider.updateMaterial(
      ...
      unitid: _selectedUnitId!,
      siteid: widget.material.siteId,
      categoryid: _selectedCategoryId!,
      materialid: widget.material.materialId,
      price: double.tryParse(_priceCtrl.text) ?? 0,
      totalamount: _totalAmount,
      supplierid: _selectedSupplierId!,
      addeddate: _dateCtrl.text,
      substageid: widget.material.substageId,
                                ...
                                children: [
                                  Text('Qty × Price',
                                      style: GoogleFonts.poppins(
                                          ...
                                          color: AppColors.greyLight)),
                                  Text(
                                    '${widget.material.qty} × ₹${widget.material.price}',
                                    style: GoogleFonts.poppins(
                                        fontSize: 13,
                                        fontWeight: FontWeight.w500,
                                        color: AppColors.greyLight),
